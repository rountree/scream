...
void AtmosphereProcessGroup::set_required_group (const std::set<Field<const Real>>& group) {
  for (auto atm_proc : m_atm_processes) {
    atm_proc->set_required_group(group);
  }
}

void AtmosphereProcessGroup::set_updated_group (const std::set<Field<Real>>& group) {
  for (auto atm_proc : m_atm_processes) {
